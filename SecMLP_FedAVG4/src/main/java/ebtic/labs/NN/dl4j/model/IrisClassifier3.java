...
import ebtic.labs.NN.dl4j.utils.DownloaderUtility;
...
public class IrisClassifier3 {

   private static Logger log = LoggerFactory.getLogger(IrisClassifier3.class);
...
//       recordReader.initialize(new FileSplit(new File(DownloaderUtility.IRISDATA.Download(),"iris.txt")));
       File inputF = new File("data/tests/IJCNN.csv");
       recordReader.initialize(new FileSplit(inputF));
       int records =0;
    // Loop through records once
       ...
       // Reinitialize to read again from the beginning
        recordReader.initialize(new FileSplit(inputF));
...
//       int numClasses = 43;     //3 classes (types of iris flowers) in the iris data set. Classes have integer values 0, 1 or 2
       ...
       //Spam
//       int labelIndex = 57;     //5 values in each row of the iris.txt CSV: 4 input features followed by an integer label (class) index. Labels are the 5th value (index 4) in each row
//       int numClasses = 2;     //3 classes (types of iris flowers) in the iris data set. Classes have integer values 0, 1 or 2
//       int batchSize = 4600;   //Iris data set: 150 examples total. We are loading all of them into one DataSet (not recommended for large data sets)

       
       //IoT23
       int labelIndex = 12;     //5 values in each row of the iris.txt CSV: 4 input features followed by an integer label (class) index. Labels are the 5th value (index 4) in each row
       int numClasses = 2;     //3 classes (types of iris flowers) in the iris data set. Classes have integer values 0, 1 or 2
          

       
       int batchSize = (int) (records *  batch);
       ...
       network.init();
       network.setListeners(new ScoreIterationListener(100)); // To print the score every 100 iterations
       
       DataSetIterator dataSetIterator = new SingletonDataSetIterator(trainingData);
       
       // Configure Early Stopping
       EarlyStoppingConfiguration<MultiLayerNetwork> esConf = new EarlyStoppingConfiguration.Builder<MultiLayerNetwork>()
//           .epochTerminationConditions(new ScoreImprovementEpochTerminationCondition(4)) // Stop after 5 epochs of no improvement
           .epochTerminationConditions(new ThresholdEpochTerminationCondition(0.004))
           .scoreCalculator(new DataSetLossCalculator(dataSetIterator, true)) // Calculate score on test data
           .evaluateEveryNEpochs(5) // Evaluate every epoch
           .modelSaver(new LocalFileModelSaver("savedModels")) // Save models to a local folder
           .build();

       // EarlyStoppingTrainer
       EarlyStoppingTrainer trainer = new EarlyStoppingTrainer(esConf, network, dataSetIterator);
...
//       System.out.println(output);
    ...
    // Get current parameters (weights and biases)
       INDArray currentParams = network.params();
       
       System.out.println("Number of parameters: "+currentParams.shape()[0]);
       
       Layer[] test = network.getLayers();
