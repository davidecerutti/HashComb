package f21bc.labs.NN.dl4j.tests;
...
public class FederatedTrainingExample {
...
public static MultiLayerConfiguration configureNetwork(int numLayers, int inputSize, int outputSize, int[] hiddenLayerSizes) {
 ...
 //       		.weightInit(new NormalDistribution(0, 0.03)) 
	...
	public static <MultiLayerNetwork, DataSetIterator> MultiLayerNetwork[] getKeysArray(HashMap<MultiLayerNetwork, DataSetIterator> map) {
    ...
    public static void main(String[] args) throws Exception {
        ...
        HashMap<MultiLayerNetwork, DataSetIterator> localModels = initializeLocalModels(numFeatures, numClasses);
    ...
    private static HashMap<MultiLayerNetwork, DataSetIterator> initializeLocalModels(int numFeatures, int numClasses) {
    	...
    	HashMap<MultiLayerNetwork, DataSetIterator> networks = new <MultiLayerNetwork, DataSetIterator>HashMap();
    ...
    private static void trainLocalModels(HashMap<MultiLayerNetwork, DataSetIterator> localModels, int epochs) {
        
    	Iterator iter = localModels.keySet().iterator();
    	while(iter.hasNext()) {
            MultiLayerNetwork localModel = (MultiLayerNetwork) iter.next();
            DataSetIterator dataIterator = localModels.get(localModel);
             
            for(int i=0; i<epochs; i++ ) {
            	
            	localModel.fit(dataIterator);  // Train the local model on its own data
            }
